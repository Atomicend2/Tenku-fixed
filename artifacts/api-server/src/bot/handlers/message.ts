...
import { BOT_OWNER_LID, PREFIX, sendText, runWithReplyContext, getBotName, isOwnerPhone } from "../connection.js";
...
export async function handleMessage(
  ...
  const isOwner = isOwnerPhone(senderPhone) || isOwnerPhone(sender.split("@")[0]);
  ...
  if (isGroup && !msg.key.fromMe) {
    // AFK should only clear on actual text/media messages — not stickers or reactions
